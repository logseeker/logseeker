...
export function Users({ auth, onChanged }: { auth: AuthStatus; onChanged: () => void }) {
  ...
  // 新規作成フォーム
  const [nu, setNu] = useState({ username: "", password: "", display_name: "", role: "viewer" as Role });
  ...
  const create = () => guard(async () => {
    ...
    setNu({ username: "", password: "", display_name: "", role: "viewer" });
    flash("ユーザーを作成しました");
  });
          ...
          <div className="card-body">
            <div className="row g-2">
              ...
              <div className="col-md-4 d-flex align-items-end">
                <button className="btn btn-primary w-100" disabled={!nu.username || !nu.password} onClick={create}>作成</button>
              </div>
            </div>
          </div>
        </div>
