...
DB管理UIが必要な場合は pgAdmin 等を別途セットアップしてください（[docs/deploy-vps-almalinux-lsws.md](docs/deploy-vps-almalinux-lsws.md) §7に構築例）。
...
### 無料で使える範囲（正規ライセンスキー未適用＝既定状態）

| 項目 | 内容 |
|---|---|
| 対応ログ種別 | Webアクセスログ / Webエラーログ（`web_access` / `web_error`）のみ |
| ティア | Tier 1（Web） |
| データ保持期間 | 既定90日（超過分は自動削除） |
| APIオプション（M365/Google Workspace等コネクタ） | 利用不可 |
| サポート | なし（セルフホスト・自己責任） |
...
| 項目 | 内容 |
|---|---|
| 対応ログ種別 | Tierに応じて段階的に解放（認証/syslog/メール → SMB/Windows/資産管理 → ルーター/ファイアウォール等。詳細は [docs/licensing.md](docs/licensing.md)） |
| データ保持期間 | 1年・3年・無制限などに延長可（[docs/retention.md](docs/retention.md)） |
| APIオプション | 有効化（M365/Google Workspace等のコネクタ取得） |
| サポート | 契約内容に応じて提供 |

> ソースを改変してこれらの制限を外すこと自体は黙認するが（[LICENSE](LICENSE) 第4条）、それは自己責任の
> セルフホスト利用として扱われ、サポート対象外になる。標準の利用方法は上記の「正規ライセンスキーを適用する」形。
...
| ドキュメント | 内容 |
|---|---|
| [INSTALL.md](INSTALL.md) | **インストール手順**。PostgreSQL/venv/systemd/nginx/firewalld/.env（Docker不使用） |
...
| [docs/licensing.md](docs/licensing.md) | ライセンス（Tier・APIオプション）による機能制御の詳細 |
