...
セットアップ手順は2種類あります。**pgAdmin（DB管理画面）を使うかどうか**で選んでください
（PostgreSQL/backendのセットアップ内容は共通、違いはリバースプロキシがnginxかApacheかだけです）。

| 手順書 | 対象 | リバースプロキシ |
|---|---|---|
| **[INSTALL.md](INSTALL.md)** | pgAdminを使わない場合（推奨・軽量） | nginx（または Caddy） |
| **[INSTALL-pgadmin.md](INSTALL-pgadmin.md)** | pgAdminも使いたい場合 | Apache（pgAdmin公式パッケージがApache前提のため） |

どちらもPostgreSQLの初期化、backend(Python/FastAPI)のvenv・systemdサービス化、frontendのビルドと配置、
リバースプロキシでのTLS設定、firewalldでのポート開放、`.env` の主要な設定項目まで一通り説明しています。
...
| コンポーネント | 役割 |
|---|---|
| nginx（または Apache／Caddy） | リバースプロキシ・TLS終端（443。`/api`,`/ingest`→backend、それ以外→frontendの静的ビルド） |
...
DB管理UI（pgAdmin）が必要な場合は [INSTALL-pgadmin.md](INSTALL-pgadmin.md) を参照してください。
...
- **検知ルール**: IOC一致・攻撃ペイロード検知・Webスキャン・危険パス・Webshell探索・認証総当たり・root SSH試行・海外アクセス・ログ未達・ビルド失敗 に加え、
...
| ドキュメント | 内容 |
|---|---|
| [INSTALL.md](INSTALL.md) | **インストール手順（pgAdminなし）**。PostgreSQL/venv/systemd/nginx/firewalld/.env（Docker不使用） |
