...
## 7. 画面ごとのIPアクセス制限（アプリ層・任意）
ログイン必須化（項番1）に加えて、管理系画面（システム状態・ユーザー管理・監査ログ・
ライセンス・通知設定・脅威インテリ）ごとに「許可したIP/CIDR以外を拒否」を設定できる。
既定は全画面OFF＝無効。

設定場所は通常のログイン後画面ではなく、`?screen=administration`（左メニューには出さない、
管理者(admin)ロール専用の別ログイン画面。[docs/auth.md](auth.md)「管理パネル」参照）から。
...
- **ロックアウト防止**：ある画面をONにする際、今アクセスしているIPが許可リストに
  含まれていないと保存自体を拒否する。Admin画面には「現在検出しているあなたのIP」が
  常に表示されるので、それを見てから許可リストに追加する。
- **万一ロックアウトした場合**：DBの`settings`テーブルから`ip_restrict_scopes`行を削除（または
  値を`[]`に更新）すれば即座に無効化できる（`psql`やpgAdminから直接操作。アプリの再起動は不要）。
