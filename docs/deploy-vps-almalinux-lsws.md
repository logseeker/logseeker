...
# 'logseeker' はユーザー名と同じ値のサンプルなので、本番では強力なパスワードに変更すること
sudo -u postgres psql <<'SQL'
CREATE USER logseeker WITH PASSWORD 'logseeker';
...
> **誤って `loghub` という名前でDB/ユーザー/`/opt/loghub`等を作成してしまった場合**、削除してから
> 上記のコマンドで `logseeker` として作り直してください。
> ```bash
> sudo -u postgres psql <<'SQL'
...
> sudo rm -rf /opt/loghub /var/www/loghub
> ```
...
環境変数ファイル `/opt/logseeker/backend/.env`（systemd から読ませる）:
```ini
DATABASE_URL=postgresql+psycopg://logseeker:logseeker@127.0.0.1:5432/logseeker
INGEST_TOKEN=
TCP_INGEST_PORT=516
GEOIP_DB_PATH=/opt/logseeker/backend/geoip/GeoLite2-Country.mmdb
JSON_STORE_DIR=/opt/logseeker/data/json
LICENSE_SECRET=
```
...
systemd ユニット `/etc/systemd/system/logseeker-backend.service`:
```ini
[Unit]
...
```bash
cd /opt/logseeker/backend
/opt/logseeker/venv/bin/python -m app.tools.load_logs --reset
```
...
1. **External App（プロキシ先）**: Type=Web Server、Address=`127.0.0.1:8000`、名前 `logseeker-backend`。
2. **Virtual Host** `logseeker`:
   - Document Root: `/var/www/logseeker`
   - **Context `/` (Static)**: 静的配信。`index.html`。SPAなので 404 を `/index.html` に rewrite（任意）。
   - **Context `/api` (Proxy)** → External App `logseeker-backend`
   - **Context `/ingest` (Proxy)** → `logseeker-backend`
   - **Context `/health` (Proxy)** → `logseeker-backend`
3. **Listener** `:80`（必要なら `:443` を Let's Encrypt 証明書付きで）→ 上記 VHost にマップ。

> 設定ファイルで書く場合は `/usr/local/lsws/conf/vhosts/logseeker/vhconf.conf` に
> `context /api/ { type proxy; handler logseeker-backend; }` のように記述（`/ingest/` `/health` も同様）。
...
sudo chown -R logseeker:logseeker /var/lib/pgadmin /var/log/pgadmin
```
...
pgAdmin の接続先 DB は Host=`127.0.0.1` Port=`5432` User=`logseeker` DB=`logseeker`。
...
- pgAdmin/logseeker のデータディレクトリのコンテキストに注意（`/var/lib/pgadmin`, `/opt/logseeker/data`）。
...
```bash
sudo systemctl enable --now postgresql-16 logseeker-backend lsws
...
- 運用更新: コード更新 → backend は `pip install -r requirements.txt` 後 `systemctl restart logseeker-backend`、frontend は `npm run build` し直して `dist` を再配置。
- バックアップ対象: PostgreSQL（`pg_dump`）、`/opt/logseeker/data`（取り込みJSON）、各 `.env` / LiteSpeed 設定。
...
```bash
sudo mkdir -p /var/backups/logseeker
sudo tee /usr/local/bin/logseeker-pg-backup.sh >/dev/null <<'EOF'
#!/bin/sh
set -eu
DEST=/var/backups/logseeker
STAMP=$(date +%Y%m%d-%H%M%S)
sudo -u postgres pg_dump -Fc logseeker > "$DEST/logseeker-$STAMP.dump"
# 7日より古い世代を削除
find "$DEST" -name 'logseeker-*.dump' -mtime +7 -delete
EOF
sudo chmod +x /usr/local/bin/logseeker-pg-backup.sh
```

**方法A: systemd timer（推奨）**
```ini
# /etc/systemd/system/logseeker-pg-backup.service
[Unit]
Description=logseeker PostgreSQL backup (pg_dump)

[Service]
Type=oneshot
ExecStart=/usr/local/bin/logseeker-pg-backup.sh
```
```ini
# /etc/systemd/system/logseeker-pg-backup.timer
[Unit]
Description=Run logseeker-pg-backup daily
...
```bash
sudo systemctl daemon-reload
sudo systemctl enable --now logseeker-pg-backup.timer
systemctl list-timers logseeker-pg-backup.timer   # 次回実行時刻の確認
```

**方法B: cron**
```bash
# root の crontab に追記（毎日3:00に実行）
echo '0 3 * * * /usr/local/bin/logseeker-pg-backup.sh' | sudo tee -a /etc/crontab
```
