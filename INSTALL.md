...
git clone https://github.com/logseeker/logseeker.git /opt/logseeker
...
`nginx`パッケージ同梱の`/etc/nginx/nginx.conf`には、既定で`server { listen 80; server_name _; ... }`
という catch-all の`server`ブロックが`http {}`内に直接入っており、これから作成する
`conf.d/logseeker.conf`の80番と衝突する（`nginx -t`が通らない、または意図しない方が優先される）。
先に無効化しておく:

```bash
sudo cp /etc/nginx/nginx.conf /etc/nginx/nginx.conf.orig   # 念のためバックアップ
sudo vi /etc/nginx/nginx.conf   # http{} 内の default の server{} ブロックをコメントアウトまたは削除
```

続けて、`sudo vi /etc/nginx/conf.d/logseeker.conf`（または`sudo nano`等）で以下の内容を作成してください。
...
# 証明書取得前は下の`server{}`ブロックごとコメントアウトしておくこと（存在しないファイルを
# ssl_certificateが指したままだと`nginx -t`が失敗し、nginxが起動できない）。
# 証明書取得手順が終わってからコメントを外し、`nginx -t && systemctl reload nginx`する。
#server {
...
まず上記の状態（443番の`server{}`はコメントアウトのまま）で起動する:
```bash
...
証明書取得（certbot、AlmaLinux/RHEL系の例）。`--nginx`は設定ファイルを自動編集した上でnginxを
再読込しようとして上のコメントアウト状態と噛み合わず失敗するため、**`certonly --webroot`を使う**
（証明書ファイルの取得のみ行い、nginx設定には触れない）:
```bash
sudo dnf -y install certbot
sudo certbot certonly --webroot -w /var/www/certbot -d your-domain.example.com
```

取得できたら`logseeker.conf`の443番`server{}`ブロックの`#`をすべて外し、反映する:
```bash
sudo nginx -t
sudo systemctl reload nginx
```

証明書の自動更新（`certbot renew`）はwebroot方式なのでnginx再起動不要。cronやsystemdタイマーで
...
その場合は上記の443番`server`ブロック・certbotの手順は不要で、80番のみで運用する。

`sudo vi /etc/nginx/conf.d/logseeker.conf`（または`sudo nano`等）で以下の内容を作成してください。
...
Cloudflare側のSSL/TLS暗号化モードは以下のいずれかにする:
- **Flexible**（最も簡単。ブラウザ⇔Cloudflareのみ暗号化、Cloudflare⇔オリジンは上記の平文HTTP）
- **Full**（Cloudflare⇔オリジンも暗号化したい場合。Let's Encryptの検証は不要な
  **Cloudflare Origin CA証明書**をCloudflareダッシュボードから発行し、上記構成に443番の
  `server`ブロックを追加してそこへ設定する。通常のcertbotの手順とは別物）

firewalldは`http`のみで足りる（`https`は開けなくてよい。Full利用時は開ける）:
```bash
sudo firewall-cmd --add-service=http --permanent
sudo firewall-cmd --reload
```

> 逆に、Cloudflareを使わず本サーバーに直接ドメインを向ける（オリジンで直接TLS終端する）場合は、
> 上の証明書取得・443番`server`ブロックの手順（本節冒頭）に従うこと。

### 代替: Caddy を使う場合

Caddy は自動でLet's EncryptのTLS証明書を取得・更新するため、設定がより簡潔です。
`sudo vi /etc/caddy/Caddyfile`（または`sudo nano`等）で以下の内容を作成してください。
