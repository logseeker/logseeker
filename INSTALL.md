...
### 5.0 共通の前提: nginxの既定設定にある衝突を解消する
...
443番の`server`ブロック・certbotの手順は丸ごと不要。80番のみで運用する。
...
1. Cloudflareダッシュボードで発行した証明書とプライベートキーをオリジンに設置する
   ...
   ```
2. `sudo vi /etc/nginx/conf.d/logseeker.conf`（または`sudo nano`等）で以下の内容を作成する:
   ```nginx
   server {
       listen 80;
       server_name your-domain.example.com;
   ...
   server {
       ...
       ssl_certificate_key /etc/pki/tls/cloudflare/logseeker-origin.key;
